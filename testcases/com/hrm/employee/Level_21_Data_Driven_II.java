...
public class Level_21_Data_Driven_II extends BaseTest {
	WebDriver driver;
	...
	@Parameters({ "browser", "url","employee" })
	@BeforeClass
	public void beforeTest(String browserName, String url, String employeeData) {
		log.info("Pre-condition - Step 01: Open browser '" + browserName + "' and navidate to '" + url + "'");
		driver = multiBrowser(browserName);
		driver.get(url);
		loginPage = PageGenerator.getLoginPage(driver);
		dataEmployee = Employee.getJson(employeeData);
	...
	public void Employee_01_Add_New_Employee() {
		...
		addEmployeePage.enterToTextboxById(driver, dataEmployee.getFirstname(), "firstName");

		log.info("Add_New_01 - Step 04: Enter valid info to Lastname textbox");
		addEmployeePage.enterToTextboxById(driver, dataEmployee.getLastname(), "lastName");
		...
		addEmployeePage.enterToTextboxById(driver, dataEmployee.getFullname(), "user_name");

		log.info("Add_New_01 - Step 08: Enter valid info into 'Password' textbox");
		addEmployeePage.enterToTextboxById(driver, dataEmployee.getUserPassword(), "user_password");

		log.info("Add_New_01 - Step 09: Enter valid info into 'Confirm password' textbox");
		addEmployeePage.enterToTextboxById(driver, dataEmployee.getUserPassword(), "re_password");

		log.info("Add_New_01 - Step 10: select 'Enabled' value in 'Status' dropdown");
		addEmployeePage.selectValueInDropdownById(driver, "status", dataEmployee.getStatusValue());
		...
		employeeListPage.enterToTextboxById(driver, dataEmployee.getFullname(), "empsearch_employee_name_empName");
		verifyTrue(employeeListPage.isJQueryAJaxLoadedSuccess(driver));
	...
	public void Employee_02_Upload_Avatar() {
		...
		dashboardPage = loginPage.loginSystem(driver, dataEmployee.getFullname(), dataEmployee.getUserPassword());
	...
	public void Employee_03_Personal_Details() {
		...
		personalDetailsPage.enterToTextboxById(driver, dataEmployee.getEditOtherId(), "personal_txtOtherID");

		log.info("Personal Details_03 - Step 11 - Enter to 'License Expiry Date' date field");
		personalDetailsPage.enterToDateFieldById(driver, dataEmployee.getLisenceExpireDate(), "personal_txtLicExpDate");
		personalDetailsPage.sleepInSecond(2);

		log.info("Personal Details_03 - Step 12 - Select 'Marital Status' dropdownlist");
		personalDetailsPage.selectValueInDropdownById(driver, "personal_cmbMarital", dataEmployee.getMaritalStatus());

		log.info("Personal Details_03 - Step 13 - Select 'Nationality' dropdownlist");
		personalDetailsPage.selectValueInDropdownById(driver, "personal_cmbNation", dataEmployee.getNationality());
		...
		verifyEquals(personalDetailsPage.getAttributeValueById(driver, "value", "personal_txtOtherID"), dataEmployee.getEditOtherId());

		log.info("Personal Details_03 - Step 17 - Verify 'License Expiry Date' is updated");
		verifyEquals(personalDetailsPage.getAttributeValueById(driver, "value", "personal_txtLicExpDate"), dataEmployee.getLisenceExpireDate());

		log.info("Personal Details_03 - Step 18 - Verify 'Marital Status' is updated");
		verifyEquals(personalDetailsPage.getSelectedItemValueById(driver, "personal_cmbMarital"), dataEmployee.getMaritalStatus());

		log.info("Personal Details_03 - Step 19 - Verify 'Nationality' dropdownlist is updated");
		verifyEquals(personalDetailsPage.getSelectedItemValueById(driver, "personal_cmbNation"), dataEmployee.getNationality());
	}
